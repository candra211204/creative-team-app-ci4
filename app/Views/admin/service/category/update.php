...
<div class="col-12">
    <div class="card recent-sales overflow-auto">
        <div class="card-body">
            ...
            <form class="row g-3" method="post" action="<?= base_url('/admin/service/category/update/' . $service['slug']); ?>">
                <?= csrf_field() ?>
                <div class="col-md-6">
                    <input type="text" class="form-control <?= array_key_exists('name', $validation) ? 'is-invalid' : ''; ?>" placeholder="Nama" name="name" value="<?= old('name') == null ? $service['name'] : old('name'); ?>" autofocus>
                    <div class="invalid-feedback">
                        <?= array_key_exists('name', $validation) ? $validation['name'] : ''; ?>
                    </div>
                </div>
                <div class="col-md-6">
                    <input type="text" class="form-control <?= array_key_exists('logo', $validation) ? 'is-invalid' : ''; ?>" placeholder="Kelas logo" name="logo" value="<?= old('logo') == null ? $service['logo'] : old('logo'); ?>">
                    <div class="invalid-feedback">
                        <?= array_key_exists('logo', $validation) ? $validation['logo'] : ''; ?>
                    </div>
                </div>
                <div class="col-12">
                    <input id="description" type="hidden" name="description" value="<?= old('description') == null ? $service['description'] : old('description'); ?>">
                ...
                <div class="text-start">
                    <a href="<?= base_url('/admin/service/category/detail/' . $service['slug']); ?>" class="btn btn-primary"><i class='bx bx-left-arrow-alt'></i></a>
